...
#ifndef ioctl_fd
#define ioctl_fd ((LONG_PTR)ioctl_handle)
#endif
...
{ Scsi_Command		cmd(ioctl_handle);	/* screw first argument */
  ...
  static int		dao_toggle=-1;
	...
	//
	if (!media_written && (mmc_profile&0xFFFF)==0x2B)
        {
            fprintf (stderr,"%s: allowing extended timeout for initial DVD+R DL write.\n",
                     ioctl_device);
            cmd.timeout(180);
        }
        else
...
{ int           err;
  ...
  unsigned char dvd_20[4+8];
    ...
    /*
     * Some USB slim drives return from SEND DVD STRUCTURE while internal
     * calibration is still settling. Do not immediately queue the first
     * WRITE(10): wait until TEST UNIT READY says the drive is ready.
     *
     * This does not write user data. It only serializes the existing layer
     * split/calibration step with the first data write.
     */
    fprintf (stderr,"%s: waiting for DVD+R DL readiness after layer split...\n",
	     ioctl_device);
    {
	long ready_started = getmsecs();
	if (wait_for_unit (cmd))
	{
	    fprintf (stderr,":-( drive did not become ready after DVD+R DL layer split\n");
	    exit (FATAL_START(errno));
	}
	fprintf (stderr,"%s: DVD+R DL ready after %ld ms\n",
		 ioctl_device,getmsecs()-ready_started);
    }
