...
EXPORT	unsigned char	*textsub;
EXPORT	int	textlen;

EXPORT	BOOL	checktextfile	__PR((char *fname));
LOCAL	void	setuptextdata	__PR((unsigned char *bp, int len));
...
LOCAL	void	eight2six	__PR((unsigned char *in, unsigned char *out));
#ifdef	__needed__
LOCAL	void	six2eight	__PR((unsigned char *in, unsigned char *out));
#endif
	...
	char	*fname;
{
	...
	for (n = 0; n < len; n += sizeof (struct textpack), tp++) {
		...
		crc ^= 0xFFFF;
		if (crc != calcCRC((unsigned char *)tp, sizeof (*tp)-2)) {
			if (cdtext_crc_ok(tp)) {
				errmsgno(EX_BAD,
	...
	int	len;
{
	...
	for (n = 0; n < len; n += sizeof (struct textpack), tp++) {
		...
		printf("CRC: %04X (%04X) ", crc, calcCRC((unsigned char *)tp, sizeof (*tp)-2));
		printf("Text: '%.12s'\n", tp->text);
	...
	struct textpack *p;
{
	...
	new.crc[0] ^= 0xFF;
	new.crc[1] ^= 0xFF;
	crc = calcCRC((unsigned char *)&new, sizeof (struct textpack));
	crc = flip_crc_error_corr((unsigned char *)&new, sizeof (struct textpack), crc);
	new.crc[0] ^= 0xFF;
	new.crc[1] ^= 0xFF;
	if (crc == 0)
		movebytes(&new, p, 18);
