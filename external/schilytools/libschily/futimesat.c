...
/*
 * RB_MINGW_UTIMES_PROTO
 * The Schily futimesat fallback calls utimes(); current MinGW headers do not
 * declare that POSIX function, and GCC 16 rejects implicit declarations.
 */
extern	int	utimes	__PR((const char *path, const struct timeval times[2]));
