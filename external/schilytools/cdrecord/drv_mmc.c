...
LOCAL	int	send_opc_mmc		__PR((SCSI *scgp, char *, int cnt, int doopc));
LOCAL	int	opt1_mmc		__PR((SCSI *scgp, cdr_t *dp));
LOCAL	int	opt2_mmc		__PR((SCSI *scgp, cdr_t *dp));
LOCAL	int	scsi_sony_write		__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
...
LOCAL int	drivemode_plextor	__PR((SCSI *scgp, char * bp, int cnt, int modecode, void *modeval));
LOCAL int	drivemode2_plextor	__PR((SCSI *scgp, char * bp, int cnt, int modecode, void *modeval));
...
EXPORT struct ricoh_mode_page_30 * get_justlink_ricoh	__PR((SCSI *scgp, unsigned char *mode));
...
cdr_t	cdr_cd = {
	...
	blank_dummy,
	format_dummy,
	(int(*)__PR((SCSI *, char *, int, int)))NULL,	/* no OPC	*/
	cmd_dummy,					/* opt1		*/
	cmd_dummy,					/* opt2		*/
};
	...
	int	excode;
{
	...
	/* RB_RETROBEAM_STREAMING_ADVANCED_HELP_30 */
	if (dp->cdr_flags & CDR_MMC3) {
		error("streamwrc=default\tUse media/firmware rotational control (default)\n");
		error("streamwrc=cav\tRequest MMC Constant Angular Velocity control\n");
		error("streamexact\tRequire the selected SET STREAMING speed exactly\n");
		error("nostreamexact\tAllow closest supported speed (default)\n");
		error("streamrestore\tRestore MMC streaming defaults (use with -setdropts)\n");
		haveopts = TRUE;
	}
...
#endif
		if (read_dvd_structure(scgp, (char *)xb, 32, 0, 0, 0, 0) >= 0) {
			/*
	...
	cdr_t			*dp;
{
	...
	if (driveropts != NULL) {
		...
		/*
		 * RB_RETROBEAM_AWSC_DRIVEROPT_34
		 *
		 * awsc=auto|force is the explicit modern spelling of the
		 * Ricoh-style AWSCD control.  Map it onto the existing
		 * RF_FORCESPEED state so speed_select_mmc() remains the single
		 * authority that applies page 30h during recording.
		 */
		p = hasdrvoptx(driveropts, "awsc", 0);
		if (p != NULL) {
			if (strncmp(p, "force", 5) == 0 &&
			    (p[5] == '\0' || p[5] == ',')) {
				if ((dp->cdr_flags & CDR_FORCESPEED) == 0)
					comerrno(EX_BAD,
					    _("Drive does not advertise Auto Write Speed Control.\n"));
				dp->cdr_dstat->ds_cdrflags |= RF_FORCESPEED;
			    ...
			    (p[6] == '\0' || p[6] == ',')) {
				/* Standalone -checkdrive query; no RF change. */
			} else {
				comerrno(EX_BAD,
				    _("Bad awsc value. Use status, auto or force.\n"));
			}
		}
...
#ifdef	DEBUG
	scg_prbytes("CD write parameter:", (unsigned char *)mode, len);
#endif
	...
	struct atipinfo *atp;
{
	...
	fillbytes((char *)atp, sizeof (*atp), '\0');
	 ...
	 */
	if (read_toc(scgp, (char *)atp, 0, sizeof (struct tocheader), 0, FMT_ATIP) < 0)
		return (-1);
	len = a_to_u_2_byte(atp->hd.len);
	len += 2;
	ret = read_toc(scgp, (char *)atp, 0, len, 0, FMT_ATIP);

#ifdef	DEBUG
	scg_prbytes("ATIP info:", (unsigned char *)atp,
...
char	atp[1024];
	 ...
	 * try to transfer only 2 bytes.
	 */
/*	if (read_toc(scgp, (char *)atp, 0, 2, 1, FMT_PMA) < 0)*/
/*	if (read_toc(scgp, (char *)atp, 0, 2, 0, FMT_PMA) < 0)*/
	if (read_toc(scgp, (char *)atp, 0, sizeof (struct tocheader), 0, FMT_PMA) < 0)
		return (-1);
/*	len = a_to_u_2_byte(atp->hd.len);*/
	len = a_to_u_2_byte(atp);
	len += 2;
/*	ret = read_toc(scgp, (char *)atp, 0, len, 1, FMT_PMA);*/
	ret = read_toc(scgp, (char *)atp, 0, len, 0, FMT_PMA);

#ifdef	DEBUG
	scg_prbytes("PMA:", (unsigned char *)atp,
				len-scg_getresid(scgp));
#endif
	ret = read_toc(scgp, (char *)atp, 0, len, 1, FMT_PMA);
	...
	struct atipinfo *atp;
{
	...
	if (scgp->verbose)
		scg_prbytes("ATIP info: ", (unsigned char *)atp, sizeof (*atp));
	...
	long	*ap;
{
	...
	if (trackp != 0 && trackp->track > 0 && is_packet(trackp)) {
		scgp->silent++;
		result = get_trackinfo(scgp, (char *)&track_info, TI_TYPE_TRACK,
							trackp->trackno,
							sizeof (track_info));
		scgp->silent--;
	}
	...
	if (result < 0) {
		if (get_trackinfo(scgp, (char *)&track_info, TI_TYPE_TRACK, 0xFF,
						sizeof (track_info)) < 0) {
	...
	if (scgp->verbose)
		scg_prbytes("track info:", (unsigned char *)&track_info,
...
/*	if (flags & F_SAO) {*/
	if (wm_base(dp->cdr_dstat->ds_wrmode) == WM_SAO) {
		...
		pad_track(scgp, dp, &trackp[1], -150, (int64_t)0,
					FALSE, 0);
#endif
	}
/*	if (flags & F_RAW) {*/
		...
		if (trackp->pregapsize > 0 && (trackp->flags & TI_PREGAP) == 0) {
			...
			if (trackp->track == 1 && is_hidden(trackp)) {
				pad_track(scgp, dp, trackp,
					trackp->trackstart-trackp->pregapsize,
					(int64_t)(trackp->pregapsize-trackp->trackstart)*trackp->secsize,
					FALSE, 0);
				...
				pad_track(scgp, dp, trackp,
					trackp->trackstart-trackp->pregapsize,
					(int64_t)trackp->pregapsize*trackp->secsize,
					FALSE, 0);
			}
		}
		return (0);
	...
	BOOL	needgap;
{
	...
	if (lverbose > 1) {
		for (i = 0; i < ncue; i++) {
			scg_prbytes("", (unsigned char *)&cue[i], 8);
		}
	}
	if (cuep)
		*cuep = cue;
	...
	track_t	*trackp;
{
	...
	scgp->silent++;
	ret = send_cue_sheet(scgp, (char *)cp, ncue*8);
	scgp->silent--;
	free(cp);
	if (ret < 0) {
	...
	SCSI	*scgp;
{
	...
	scgp->silent++;
	if (drivemode_plextor(scgp, (char *)pgetmode, sizeof (pgetmode), modecode, NULL) < 0) {
		scgp->silent--;
		return (-1);
	}
	scgp->silent--;
	...
	SCSI	*scgp;
{
	...
	fillbytes(pgetmode, sizeof (pgetmode), '\0');
	scgp->silent++;
	if (drivemode2_plextor(scgp, (char *)pgetmode, sizeof (pgetmode), modecode, NULL) < 0) {
		scgp->silent--;
		return (-1);
	}
	scgp->silent--;
	...
	int	*lastp;
{
	...
	fillbytes((char *)scmd, sizeof (*scmd), '\0');
	fillbytes((char *)buf, sizeof (buf), '\0');
	scmd->flags = SCG_DISRE_ENA;
	scmd->flags |= SCG_RECV_DATA;
	scmd->addr = buf;
 ...
 *
 * Reusable AWSC page-30 accessors for transactional recording.
 * Return values:
 *   0  query/set completed
 *   1  page/control unavailable
 *  -1  transport/readback failure
 */
EXPORT int
retrobeam_awsc_get_state(SCSI *scgp, int *supportedp, int *awscdp)
{
	unsigned char	mode[0x100];
	struct ricoh_mode_page_30	*rp;
...
EXPORT int
retrobeam_awsc_set_state(SCSI *scgp, int awscd)
{
	unsigned char	mode[0x100];
	struct ricoh_mode_page_30	*rp;
	int		mode_len;
	...
	rp->AWSCD = awscd ? 1 : 0;
	mode_len = (int)mode[0] + 1;

	if (!set_mode_params(scgp, _("Ricoh Vendor Page"), mode,
	    mode_len, 0, -1))
		return (-1);
...
/*
 * RB_RETROBEAM_AWSC_CONTROL_33
 *
 * Standalone Ricoh-style Auto Write Speed Control handling.
 *
 * Unlike the historical 'forcespeed' path inside speed_select_*(), this
 * command does not touch media write parameters or select a recording speed.
 * It reads/writes only vendor mode page 30h and immediately verifies the
 * resulting AWSCD bit by reading the page again.
 *
 * driveropts:
 *   awsc=status  query only
 *   awsc=auto    AWSCD=0: adaptive write-speed control enabled
 *   awsc=force   AWSCD=1: adaptive write-speed control disabled
 */
EXPORT int
retrobeam_awsc_control(SCSI *scgp)
{
	unsigned char	mode[0x100];
	struct ricoh_mode_page_30	*rp;
	char		*p;
	...
	if (rp == NULL) {
		printf(_("RetroBeam AWSC: mode page 0x30 unavailable.\n"));
		printf(_("[RBAWSC] supported=0 result=page_unavailable\n"));
		return (1);
	}

	original = rp->AWSCD ? 1 : 0;

	printf(_("RetroBeam AWSC: AWSCS=%u AWSCD=%u (%s).\n"),
	    rp->AWSCS ? 1U : 0U,
	    rp->AWSCD ? 1U : 0U,
	    rp->AWSCD ?
		_("adaptive write-speed control disabled / force") :
		_("adaptive write-speed control enabled / auto"));

	printf(_("[RBAWSC] supported=%u awscs=%u awscd=%u mode=%s\n"),
	    rp->AWSCS ? 1U : 0U,
	    rp->AWSCS ? 1U : 0U,
	    rp->AWSCD ? 1U : 0U,
	    rp->AWSCD ? "force" : "auto");
	...
	if (!rp->AWSCS) {
		printf(_("RetroBeam AWSC: page exists but AWSCS=0; control is not advertised.\n"));
	...
	if (request == original) {
		printf(_("RetroBeam AWSC: requested state is already active.\n"));
		printf(_("[RBAWSC] set_requested=%u changed=0 verified=1\n"),
		    (unsigned int)request);
		return (0);
	}

	rp->AWSCD = request ? 1 : 0;

	/*
	 * get_mode_params() returns the compact mode-parameter block used by
	 * existing cdrecord Ricoh-page setters.  Preserve every other byte.
	 */
	mode_len = (int)mode[0] + 1;
	...
	if (!set_mode_params(scgp, _("Ricoh Vendor Page"), mode,
	    mode_len, 0, -1)) {
		errmsgno(EX_BAD,
		    _("RetroBeam AWSC: MODE SELECT page 0x30 failed.\n"));
		printf(_("[RBAWSC] set_requested=%u changed=0 verified=0 result=mode_select_failed\n"),
		    (unsigned int)request);
		return (-1);
	}
	...
	if (rp == NULL) {
		errmsgno(EX_BAD,
		    _("RetroBeam AWSC: readback failed after MODE SELECT.\n"));
		printf(_("[RBAWSC] set_requested=%u changed=1 verified=0 result=readback_failed\n"),
		    (unsigned int)request);
		return (-1);
	}
	...
	printf(_("RetroBeam AWSC readback: AWSCS=%u AWSCD=%u (%s).\n"),
	    rp->AWSCS ? 1U : 0U,
	    rp->AWSCD ? 1U : 0U,
	    rp->AWSCD ?
		_("adaptive write-speed control disabled / force") :
	...
	printf(_("[RBAWSC] set_requested=%u changed=1 verified=%u readback_awscd=%u mode=%s\n"),
	    (unsigned int)request,
	    verify == request ? 1U : 0U,
	    (unsigned int)verify,
	    verify ? "force" : "auto");

	if (verify != request) {
		errmsgno(EX_BAD,
		    _("RetroBeam AWSC: drive did not retain requested AWSCD state.\n"));
		return (-1);
	}
