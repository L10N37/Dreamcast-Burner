...
struct teac_mode_page_21 {		/* teac dummy selection */
	...
	unsigned char	p_len;			/* 0x01 = 1 Byte */
...
struct teac_mode_page_31 {		/* teac speed selection */
		MP_P_CODE;		/* parsave & pagecode */
	unsigned char	p_len;			/* 0x02 = 2 Byte */
	unsigned char	speed;
	unsigned char	res;
};
...
LOCAL	int	write_teac_xg1		__PR((SCSI *scgp, char *, long, long, int, BOOL));
LOCAL	int	cdr_write_teac		__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
...
LOCAL	int	teac_opc		__PR((SCSI *scgp, char *, int cnt, int doopc));
...
LOCAL	int	set_subcode		__PR((SCSI *scgp, unsigned char *subcode_data, int length));
#ifdef	XDI
LOCAL	int	read_disk_info_teac	__PR((SCSI *scgp, unsigned char *data, int length, int type));
#endif
	...
	cdr_t	*dp;
{
	...
	if (mode_sense(scgp, (unsigned char *)&md, count, page, 0) < 0) {	/* Page n current */
		return (-1);
	} else {
	...
	int	*speedp;
{
	...
	status = mode_select(scgp, (unsigned char *)&md, count, 0, scgp->inq->data_format >= 2);
	if (status < 0)
		return (status);
...
if (trackp->pregapsize != 0) {
	...
	if (lverbose > 1)
		scg_prbytes(_("Subcode:"), (unsigned char *)&sc, sizeof (sc));

	status = set_subcode(scgp, (unsigned char *)&sc, sizeof (sc));
	if (status < 0)
		return (status);
	...
	if (pad_track(scgp, dp, trackp,
			lba_addr, (int64_t)pregapsize*trackp->secsize,
			FALSE, (int64_t *)0) < 0)
		return (-1);
}
...
/*		errmsgno(EX_BAD, "Cannot fixate zero track disk.\n");*/
	...
	status = sp - ((unsigned char *)buf);
	if (lverbose > 1) {
		printf(_("Subcode len: %d\n"), status);
		scg_prbytes(_("Subcode:"), (unsigned char *)buf, status);
	}
	status = set_subcode(scgp, (unsigned char *)buf, status);
	rb_sleep_seconds(1);
	if (status < 0)
		return (status);
...
set_subcode(scgp, subcode_data, length)
	SCSI	*scgp;
	unsigned char	*subcode_data;
	int	length;
{
	register struct	scg_cmd	*scmd = scgp->scmd;

	fillbytes((char *)scmd, sizeof (*scmd), '\0');
	scmd->addr = (char *)subcode_data;
	scmd->size = length;
	scmd->flags = SCG_DISRE_ENA;
	scmd->cdb_len = SC_G1_CDBLEN;
	scmd->sense_len = CCS_SENSE_LEN;
