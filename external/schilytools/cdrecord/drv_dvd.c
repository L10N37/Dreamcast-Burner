...
LOCAL	int	send_opc_dvd		__PR((SCSI *scgp, char *bp, int cnt, int doopc));
...
#ifdef	__needed__
LOCAL	int	read_rzone_info		__PR((SCSI *scgp, char * bp, int cnt));
LOCAL	int	reserve_rzone		__PR((SCSI *scgp, long size));
#endif
/*LOCAL	int	send_dvd_structure	__PR((SCSI *scgp, char * bp, int cnt));*/
#ifdef	__needed__
...
cdr_t	cdr_dvd = {
	...
	select_secsize,
	next_wr_addr_dvd,
	(int(*)__PR((SCSI *, unsigned long)))cmd_ill,		/* reserve_track */
	scsi_cdr_write,
	...
	blank_dvd,
	format_dummy,
	send_opc_dvd,					/* RetroBeam DVD-minus OPC */
	cmd_dummy,					/* opt1		*/
	cmd_dummy,					/* opt2		*/
};
...
/*
 * RB_RETROBEAM_DVD_MINUS_OPC_24
 *
 * The inherited generic DVD-minus driver disabled OPC entirely.
 * Modern DVD-R/DVD-RW writing should give the drive an explicit MMC
 * SEND OPC INFORMATION opportunity before user data is written.
 *
 * Scope this to writable DVD-minus profiles only. DVD-RAM is excluded.
 */
/*
 * RB_RETROBEAM_DVD_MINUS_OPC_24
 * RB_RETROBEAM_OPC_POLICY_26
 *
 * Explicit DVD-minus OPC policy.
 *
 * Default "auto" follows the useful growisofs policy:
 *   - READ DISC INFORMATION;
 *   - if no prior OPC descriptors are reported, request explicit OPC;
 *   - if descriptors already exist, leave calibration to the firmware.
 *
 * Expert overrides:
 *   driveropts=opc=force (or bare opc) -> always request explicit OPC
 *   driveropts=opc=skip  (or noopc)    -> never send explicit OPC
 *
 * This affects only writable DVD-minus profiles handled by mmc_dvd.
 */
LOCAL int
send_opc_dvd(scgp, bp, cnt, doopc)
	SCSI	*scgp;
	char	*bp;
	int	cnt;
	int	doopc;
{
	...
	char		*p;
	int		profile;
	int		policy = 0;	/* 0 auto, 1 force, 2 skip */
	int		opc_entries = -1;
	...
	/*
	 * Parse RetroBeam policy.  Keep compatibility aliases short and
	 * convenient for scripted/backend use.
	 */
	if (driveropts != NULL) {
		p = hasdrvopt(driveropts, "noopc");
		if (p != NULL && *p == '1')
			policy = 2;

		p = hasdrvoptx(driveropts, "opc", 0);
		if (p != NULL && *p != '\0') {
			if (strncmp(p, "auto", 4) == 0 &&
				...
				errmsgno(EX_BAD,
				    _("Bad OPC policy '%s' (use auto, force, or skip).\n"),
				    p);
				return (-1);
			}
		} else {
			p = hasdrvopt(driveropts, "opc");
			if (p != NULL && *p == '1')
				policy = 1;
		}
	}
	...
	/*
	 * Read current disc information even for explicit modes so diagnostics
	 * tell us whether the inserted media already carries OPC descriptors.
	 */
	fillbytes((char *)&di, sizeof (di), '\0');
	if (get_diskinfo(scgp, &di, sizeof (di)) >= 0)
		opc_entries = di.num_opc_entries;

	if (policy == 2) {
		printf(_("RetroBeam OPC: policy=skip existing_descriptors=%d action=skip.\n"),
		    opc_entries);
		return (0);
	}

	if (policy == 0 && opc_entries > 0) {
		printf(_("RetroBeam OPC: policy=auto existing_descriptors=%d action=skip.\n"),
		    opc_entries);
		return (0);
	}

	if (policy == 1) {
		printf(_("RetroBeam OPC: policy=force existing_descriptors=%d action=perform.\n"),
		    opc_entries);
	} else {
		printf(_("RetroBeam OPC: policy=auto existing_descriptors=%d action=perform.\n"),
		    opc_entries);
	}

	return (send_opc(scgp, bp, cnt, doopc));
}
LOCAL cdr_t *
	...
	cdr_t	*dp;
{
	...
	/*
	 * RB_RETROBEAM_BURNFREE_DEFAULT_25
	 *
	 * Buffer-underrun protection is a drive capability, not an exotic
	 * opt-in feature on a modern writer. Enable it by default when the
	 * drive advertises support. The existing driveropts parser below
	 * still lets an explicit "noburnfree" request turn it back off.
	 */
	...
	if (driveropts != NULL) {
		...
		if (strcmp(driveropts, "help") == 0) {
			...
			printf(_("  opc=auto\tRetroBeam DVD-minus OPC automatic policy (default)\n"));
			printf(_("  opc=force\tForce explicit SEND OPC INFORMATION before write\n"));
			printf(_("  opc=skip\tSkip explicit OPC (expert compatibility)\n"));
			printf(_("  noopc\t\tAlias for opc=skip\n"));
		}
...
again:
	...
	read_rzone_info(scgp, (char *)rp, sizeof (struct rzone_info));
	...
	movebytes(mode, (char *)&rz, sizeof (struct rzone_info));
	 ...
	 */
	fillbytes((char *)mode, sizeof (mode), '\0');
	if (read_dvd_structure(scgp, (char *)mode, 2, 0, 0, 0, 0) < 0) {
	...
	read_dvd_structure(scgp, (char *)sp, len, 0, 0, 0, 0);
/*	if (lverbose > 1)*/
/*		print_dvd00(sp);*/
	...
	int	*speedp;
{
...
#ifdef	DEBUG
	if (lverbose > 1)
		scg_prbytes(_("CD write parameter:"), (unsigned char *)mode, len);
#endif
	 ...
	 */
	val = curspeed * 1385; /* RB_RETROBURNER_DVD_NOMINAL_SPEED_23 */
	if (val > 0x7FFFFFFF)
		val = 0x7FFFFFFF;
	...
	long	*ap;
{
	...
	if (trackp == 0) {
		...
		if (get_trackinfo(scgp, (char *)&rz, TI_TYPE_TRACK, tracks, sizeof (rz)) < 0)
			return (-1);
		if (!rz.nwa_v)
			return (-1);
	...
	if (trackp->track <= 1) {
		...
		if (track_base(trackp)->tracktype & TOCF_MULTI)
			get_trackinfo(scgp, (char *)&rz, TI_TYPE_TRACK, trackp->trackno, sizeof (rz));
		else
			read_rzone_info(scgp, (char *)&rz, sizeof (struct rzone_info));
	...
	track_t *trackp;	/* Called with &track[1] */
{
	...
	for (i = 0; i < MAX_TRACK; i++) {
		...
		amount += (int64_t)trackp[i].padsecs * secsize;
		sectors += amount/secsize;
		ttrsize += trackp[i].tracksize;
		tamount += amount;
		if (vtracks && lverbose)
			printf(_("Vtrack:  %d size: %lld bytes %lld rounded (%lld sectors)\n"),
				(int)trackp[i].track, (int64_t)trackp[i].tracksize,
				amount, amount / (int64_t)secsize);
	...
	track_t	*trackp;
{
	...
	amount += (int64_t)trackp->padsecs * secsize;
	sectors += amount/secsize;
...
#define	W_SLEEP	2
	...
	for (i = 0; i < secs/W_SLEEP; i++) {
		...
		if (key != SC_UNIT_ATTENTION && key != SC_NOT_READY)
			break;
#ifdef	DVD_DEBUG
		request_sense_b(scgp, (char *)sensebuf, sizeof (sensebuf));
#ifdef	XXX
	...
	track_t	*trackp;
{
	...
	/*
	 * RB_RETROBURNER_DVDR_IMMED_FLUSH_07
	 *
	 * ImgBurn's observed DVD-R post-write SYNCHRONIZE CACHE uses IMMED=1.
	 * Scope this experiment to the generic DVD-R DAO lead-out path only.
	 *
	 * waitformat() immediately follows this command, so completion is still
	 * explicitly waited for after the drive accepts the immediate flush.
	 *
	 * Do not alter CD, DVD+, BD, abort handling, or generic scsi_flush_cache().
	 */
	...
	int32_t	lbreak;
{
	 ...
	 */
	fillbytes((char *)&jz, sizeof (jz), '\0');
	ret = read_dvd_structure(scgp, (char *)&jz, sizeof (jz), 0, 0, 0, 0x22);
	if (ret < 0)
		return (ret);
...
/*		jump_lba -= 1;*/
...
#endif
		ret = send_dvd_structure(scgp, (char *)&jz, sizeof (jz), 0x22);
		if (ret < 0)
			return (ret);
	}
#endif	/* USE_STRUC_22 */
	 ...
	 */
	fillbytes((char *)&lb, sizeof (lb), '\0');
	ret = read_dvd_structure(scgp, (char *)&lb, sizeof (lb), 0, 0, 0, 0x23);
	if (ret < 0)
		return (ret);
	...
	ret = send_dvd_structure(scgp, (char *)&lb, sizeof (lb), 0x23);
	return (ret);
}
#endif
	...
	struct dvd_structure_0E *dp;
{
	...
	printf(_("part v./ext code:%X/%X\n"), (unsigned int)(dp->res_a[0] & 0xF0) >> 4,
						dp->res_a[0] & 0xF);
...
/*	send_dvd_structure(scgp, (char *)&d, sizeof (d));*/
}
#endif
...
/*		scg_prbytes("Mode: ", mode, 255 - scg_getresid(scgp));*/
	...
	if (lverbose > 1) {
		...
		for (i = 0; i <= 255; i++) {
			fillbytes((char *)mode, sizeof (mode), '\0');
			ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, i);
...
/*		printf("Page: %d ret: %d len: %d\n", i, ret, sizeof (mode) - scg_getresid(scgp));*/
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
...
/*			scg_prascii("DVD structure[0]: ", mode, sizeof (mode) - scg_getresid(scgp));*/
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0x11);
	scgp->silent--;
	if (ret >= 0) {
		adip_t	*adp;
		if (lverbose > 2) {
...
/*		printf("size %d %d\n", sizeof (adip_t), offsetof(adip_t, res_controldat));*/
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0x20);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0x22);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0x23);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 1);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 4);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 5);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0xD);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0xE);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
...
/*	send_dvd0F();*/
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 0, 0, 0, 0xF);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	read_rzone_info(scgp, (char *)mode, sizeof (mode));
	if (lverbose > 2)
		scg_prbytes(_("Rzone info: "), mode, sizeof (mode) - scg_getresid(scgp));
	przone((struct rzone_info *)mode);

	scgp->verbose++;
	log_sense(scgp, (char *)mode, 255, 0x3, 1, 0);
	log_sense(scgp, (char *)mode, 255, 0x31, 1, 0);
	scgp->verbose--;
	...
	SCSI	*scgp;
{
	 ...
	 */
	scgp->silent++;
	fillbytes((char *)log, sizeof (log), '\0');
	if (get_log(scgp, (char *)log, &len, 0x30, LOG_CUMUL, 0) < 0) {
		scgp->silent--;
		return;
	}
	scgp->silent--;
	...
	scgp->silent++;
	fillbytes((char *)log, sizeof (log), '\0');
	if (get_log(scgp, (char *)log, &len, 0x30, LOG_CUMUL, 1) < 0) {
		scgp->silent--;
		return;
	}
	scgp->silent--;
	...
	scgp->silent++;
	fillbytes((char *)log, sizeof (log), '\0');
	if (get_log(scgp, (char *)log, &len, 0x30, LOG_CUMUL, 2) < 0) {
		scgp->silent--;
		return;
	}
	scgp->silent--;
