...
/*
 * As int64_t is currently a 'best effort' long long, we usually need to
...
LOCAL	void	prnum  __PR((unsigned long, unsigned, f_args *));
LOCAL	void	prdnum __PR((unsigned long, f_args *));
LOCAL	void	pronum __PR((unsigned long, f_args *));
LOCAL	void	prxnum __PR((unsigned long, f_args *));
LOCAL	void	prXnum __PR((unsigned long, f_args *));
#ifdef	USE_LONGLONG
LOCAL	void	prlnum  __PR((uint64_t, unsigned, f_args *));
LOCAL	void	prldnum __PR((uint64_t, f_args *));
LOCAL	void	prlonum __PR((uint64_t, f_args *));
LOCAL	void	prlxnum __PR((uint64_t, f_args *));
LOCAL	void	prlXnum __PR((uint64_t, f_args *));
#endif
		...
		case 'n':
			...
			case 'L': {
				...
				*lp = count;
				...
				continue;
#ifdef	USE_LONGLONG
			case 'J':		/* For now intmax_t is int64_t */
			case 'Q': {
				int64_t *qp = va_arg(args, int64_t *);
