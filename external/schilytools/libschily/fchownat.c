...
/*
 * RB_MINGW_CHOWN_FALLBACK_PROTO
 * Modern MinGW has no CRT chown() declaration. libschily/chown.c supplies
 * Schily's fallback when HAVE_CHOWN is absent, so make that existing
 * fallback visible to this translation unit.
 */
#ifndef	HAVE_CHOWN
extern	int	chown	__PR((const char *path, uid_t owner, gid_t group));
#endif
