...
/* RB_RETROBURNER_CDRTOOLS_BUILD_03 */
LOCAL	char	rb_retrobeam_build[] = "RetroBeam Optical Recording Engine 0.6";
...
EXPORT	int	pad_track	__PR((SCSI *scgp, cdr_t *dp,
					track_t *trackp,
					long startsec, int64_t amt,
					BOOL dolast, int64_t *bytesp));
EXPORT	int	write_buf	__PR((SCSI *scgp, cdr_t *dp,
					track_t *trackp,
					char *bp, long startsec, int64_t amt,
					int secsize,
					BOOL dolast, int64_t *bytesp));
...
struct exargs {
	...
	/* RB_RETROBEAM_AWSC_BURN_TRANSACTION_34 */
	int		rb_awsc_restore_pending;
	int		rb_awsc_original;
} exargs;
	...
	char	*av[];
{
	 ...
	 * privilege handling, libscg open, or any optical-drive access.
	 */
	if (getenv("RETROBURNER_FIFO_SELFTEST") != NULL) {
		if (rb_fifo_selftest())
			return (0);
		error("RetroBeam Win32 FIFO self-test: FAIL\n");
		return (1);
	}
#endif
...
#if	defined(__MINGW32__) && !defined(__CYGWIN__)
	/* RB_RETROBURNER_SINGLE_RUNTIME_BANNER_21 */ 	printf("%s\n", rb_retrobeam_build);
#endif	/*
...
#define	TEXT_DOMAIN "cdrecord"	/* Use this only if it weren't */
    ...
    /*
     * RB_RETROBURNER_CLEAN_CONSOLE_13
     *
     * RetroBurner is an independently maintained recording-engine fork.
     * Original cdrtools/cdrecord authorship remains credited here.
     * Copyright and license notices remain in the source distribution.
     */
    if ((flags & F_MSINFO) == 0 || lverbose || flags & F_VERSION) {
        printf("Core: derived from cdrtools cdrecord %s by Joerg Schilling\n", cdr_version);
        /* RB_RETROBURNER_SCHILLING_MEMORIAL_14 */
        if (flags & F_VERSION)
            printf("In memory of Joerg Schilling, whose cdrtools work remains at the heart of RetroBeam.\n");
        if (debug)
            printf("Backend: RetroBeam optical recording engine\n");
    }
...
#endif /* !_WIN32: POSIX privilege capability warnings */
	/*
	...
	if ((flags & F_MSINFO) == 0 || lverbose) {
		 ...
		 */
		/*
		 * RB_RETROBEAM_TRANSPORT_IDENTITY_27
		 *
		 * The inherited libscg QA machinery intentionally exposes author,
		 * internal version and SCCS strings when the library is modified.
		 * That provenance belongs in explicit debug output, not normal burns.
		 */
#ifdef	_WIN32
		printf(_("Transport: RetroBeam Windows SPTI\n"));
#elif	defined(__APPLE__)
		printf(_("Transport: RetroBeam macOS optical transport\n"));
#else
		printf(_("Transport: RetroBeam POSIX optical transport\n"));
#endif

		if (debug > 0) {
			vers = scg_version(0, SCG_VERSION);
			auth = scg_version(0, SCG_AUTHOR);
			error(_("Debug transport core: modified libscg %s-%s\n"),
			    auth ? auth : "unknown",
			    vers ? vers : "unknown");
		}
		if (debug > 1) {
			...
			sccs = scg_version(0, SCG_SCCS_ID);
			if (sccs != NULL)
				error(_("Debug libscg provenance: %s\n"), sccs);
		}
...
/*	bufsize = scg_bufsize(scgp, CDR_BUF_SIZE);*/
	...
	if (lverbose || debug)
		error(_("SCSI transfer buffer: %ld bytes\n"), bufsize);
	...
	exargs.flags	   = flags;	exargs.rb_awsc_restore_pending = 0;
	exargs.rb_awsc_original = 0;
	...
	/* RB_RETROBURNER_NO_LEGACY_DMA_BENCHMARK_19 	 * Modern transports are not benchmarked by repeatedly reading 	 * the recorder's internal buffer. FIFO and MMC buffer-capacity 	 * telemetry remain active and are the useful runtime signals. 	 */
	...
	if ((flags & F_CHECKDRIVE) != 0) {
		/*
		 * RB_RETROBEAM_AWSC_CHECKDRIVE_33
		 *
		 * Vendor drive controls that do not require mounted media belong
		 * before the historical -checkdrive early exit.
		 */
		if (driveropts != NULL &&
		    hasdrvoptx(driveropts, "awsc", 0) != NULL) {
			if (retrobeam_awsc_control(scgp) < 0)
				comexit(EX_BAD);
		}
		exit(0);
	}
...
/*			flags &= F_TAO;*/
	...
	if (tracks > 1)
		rb_sleep_seconds(2);	/* Let the user watch the inquiry messages */
	...
	/*
	 * RB_RETROBEAM_AWSC_SNAPSHOT_34
	 *
	 * Snapshot the drive's pre-burn AWSCD state before any recording
	 * path can alter it.  The exit handler restores this exact value.
	 */
	if ((flags & F_WRITE) != 0 && driveropts != NULL) {
		char	*rb_awsc;
		int	rb_supported;
		int	rb_original;

		rb_awsc = hasdrvoptx(driveropts, "awsc", 0);
		if (rb_awsc != NULL) {
			if (strncmp(rb_awsc, "status", 6) == 0 &&
			    (rb_awsc[6] == '\0' || rb_awsc[6] == ','))
				comerrno(EX_BAD,
				    _("awsc=status is query-only and cannot be used for recording.\n"));

			if (retrobeam_awsc_get_state(scgp,
			    &rb_supported, &rb_original) != 0 || !rb_supported)
				comerrno(EX_BAD,
				    _("Drive does not advertise writable Auto Write Speed Control.\n"));

			exargs.rb_awsc_original = rb_original;
			exargs.rb_awsc_restore_pending = 1;

			printf(_("RetroBeam AWSC transaction: original=%s, requested=%s.\n"),
			    rb_original ? _("force") : _("auto"),
			    (strncmp(rb_awsc, "force", 5) == 0) ?
				_("force") : _("auto"));
			printf(_("[RBAWSC] transaction original=%u restore_pending=1\n"),
			    (unsigned int)rb_original);
		}
	}

	/*
	 * RB_RETROBEAM_VENDORPROBE_NOMEDIA_33
	 *
	 * driveropts=vendorprobe is a query-only discovery operation.
	 * Run it before load_media() so an empty tray can still expose drive
	 * GET CONFIGURATION and MODE SENSE state. No media command is required.
	 */
...
/*return 0;*/
	...
	if (flags & F_MEDIAINFO) {
		...
		retrobeam_media_intelligence(scgp);
		comexit(0);
	}
	if (flags & F_MSINFO) {
...
{
	/*
	 * RB_RETROBURNER_NO_GRACE_COUNTDOWN_11
	 *
	 * RetroBurner owns the confirmation UI.  The command-line engine no
	 * longer adds cdrecord's historical grace-time countdown after the
	 * caller has already committed to a burn.
	 *
	 * Preserve the interrupt handlers used once recording is active.
	 */
	printf("RetroBeam: starting %s %s write at %dx, %s session.\n",
	...
	void	*arg;
{
	...
	BOOL	rb_media_mutating = FALSE;
	BOOL	rb_skip_cleanup_flush = FALSE;

	/*
	 * RB_RETROBURNER_MMC_DVD_SUCCESS_NO_CLEANUP_FLUSH_08
	 * RB_RETROBEAM_READONLY_NO_MEDIA_CLEANUP_29
	 *
	 * exscsi() is registered as generic process cleanup, so upstream called
	 * SYNCHRONIZE CACHE and cdr_abort_session() even after information-only
	 * commands such as -toc, -atip and -minfo.
	 *
	 * Modern RetroBeam separates media-mutating cleanup from read-only
	 * teardown.  A query must not issue a cache flush or abort a recording
	 * session that it never opened.
	 *
	 * F_WRITE, F_BLANK, F_FORMAT and F_FIX identify operations that may have
	 * changed media state and therefore retain the inherited defensive
	 * cleanup on error/abort.
	 *
	 * A successful mmc_dvd write remains the already-proven special case:
	 * fixate_dvd() has completed its explicit final SYNCHRONIZE CACHE, so
	 * the historical second cleanup flush stays suppressed.
	 */
	if (exp != NULL) {
		rb_media_mutating =
		    (exp->flags & (F_WRITE|F_BLANK|F_FORMAT|F_FIX)) != 0;
	}

	if (!rb_media_mutating) {
		rb_skip_cleanup_flush = TRUE;
	} else if (excode == 0 &&
	    exp != NULL &&
	    exp->dp != NULL &&
	    exp->dp->cdr_drname != NULL &&
	    (exp->flags & F_WRITE) != 0 &&
	    strcmp(exp->dp->cdr_drname, "mmc_dvd") == 0) {
		rb_skip_cleanup_flush = TRUE;
	}

	/*
	 * Restore non-media state and perform media cleanup only where needed.
	 */
	if (exp != NULL && exp->exflags == 0) {
		...
		/*
		 * RB_RETROBEAM_AWSC_RESTORE_34
		 *
		 * Restore the pre-burn drive policy after both successful and
		 * failed writes.  This is drive state, not media state.
		 */
		if (exp->rb_awsc_restore_pending) {
			int	rb_restore_ret;

			rb_restore_ret = retrobeam_awsc_set_state(exp->scgp,
			    exp->rb_awsc_original);

			if (rb_restore_ret == 0) {
				printf(_("RetroBeam AWSC transaction: restored original %s state.\n"),
				    exp->rb_awsc_original ? _("force") : _("auto"));
				printf(_("[RBAWSC] transaction restored=1 awscd=%u\n"),
				    (unsigned int)exp->rb_awsc_original);
			} else {
				errmsgno(EX_BAD,
				    _("Warning: RetroBeam could not restore original AWSC state.\n"));
				printf(_("[RBAWSC] transaction restored=0 requested_awscd=%u result=%d\n"),
				    (unsigned int)exp->rb_awsc_original,
				    rb_restore_ret);
			}

			exp->rb_awsc_restore_pending = 0;
		}
	...
	if (amount < 0) {
		if (scsi_in_progress(scgp)) {
			...
			if ((dp->cdr_dstat->ds_flags & DSF_NOCD) == 0) {
				rb_sleep_us(60000);	/* CD case */
			} else {
#ifndef	_SC_CLK_TCK
				rb_sleep_us(20000);	/* DVD/BD case */
#else
...
/*
 * RB_RETROBURNER_DVD_BCAP_64M_06
 *
 * Upstream cdrecord couples READ BUFFER CAPACITY to its 1 MiB verbose
 * progress display.  On DVD/BD this causes one extra SCSI command per
 * displayed MiB.  ImgBurn's observed DVD-R write cadence is approximately
 * one drive-buffer query per 64 MiB of payload.
 *
 * Keep progress output at 1 MiB, but cache the last drive-buffer percentage
 * and refresh it every 64 MiB for normal DVD/BD writes.
 *
 * Do NOT throttle:
 *   - CD media (preserve upstream behaviour)
 *   - RF_WR_WAIT, where the fresh buffer reading is used for active pacing.
 */
int64_t rb_bcap_next = 0;
int64_t rb_bcap_interval = (int64_t)64 * 1024 * 1024;
BOOL rb_bcap_throttle = FALSE;
#ifdef	XBCAP
int oper = -1;
#endif
#endif
...
#ifdef	BCAP
	if (dp->cdr_dstat->ds_flags & (DSF_DVD|DSF_BD))
		rb_bcap_throttle = TRUE;
#endif
	...
	do {
		...
		if (lverbose && (bytes >= (savbytes + 0x100000))) {
			...
			if (bsize > 0 && bytes > bsize) { /* buffer size known */
				BOOL	rb_do_bcap = TRUE;

				if (rb_bcap_throttle &&
				    (dp->cdr_dstat->ds_cdrflags & RF_WR_WAIT) == 0 &&
				    rb_bcap_next > 0 &&
				    bytes < rb_bcap_next) {
					rb_do_bcap = FALSE;
				}
				...
				/*
				 * Keep the existing live [buf N%] UI on every 1 MiB
				 * progress line, using the most recently sampled value.
				 */
				if (per >= 0)
					printf(_(" [buf %3d%%]"), per);
#ifdef	BCAPDBG
	...
	if (trackp->padsecs > 0) {
		 ...
		 */
		padbytes = (int64_t)trackp->padsecs * secsize;
...
int oper = -1;
	...
	do {
		...
		if (is_raw(trackp)) {
			encsectors(trackp, (unsigned char *)buf, startsec, secspt);
			fillsubch(trackp, (unsigned char *)buf, startsec, secspt);
		}
	...
	track_t	*trackp;
{
	if (trackp->itracksize >= 0) {
		printf(_("Track %02d: data  %4lld MB        "),
					track, (int64_t)(trackp->itracksize >> 20));
	} else {
		printf(_("Track %02d: data  unknown length"),
					track);
	}
	if (trackp->padsecs > 0) {
		int64_t	padbytes = (int64_t)trackp->padsecs * trackp->isecsize;

		if ((padbytes >> 20) > 0)
			printf(_(" padsize: %4lld MB"), (int64_t)(padbytes >> 20));
		else
			printf(_(" padsize: %4lld KB"), (int64_t)(padbytes >> 10));
	}
	...
	track_t	*trackp;
{
	if (trackp->itracksize > 0 &&
		...
		errmsgno(EX_BAD, _("Bad audio track size %lld for track %02d.\n"),
				(int64_t)trackp->itracksize, track);
	...
	track_t	*trackp;
{
	int	i;
	int64_t	curr;
	int64_t	total = -150;	/* CD track #1 pregap compensation */
	uint64_t	btotal;
	track_t	*tp;
	...
	if (trackp->flags & TI_NOCD)
		btotal = (uint64_t)total * 2048;
	else
		btotal = (uint64_t)total * 2352;
/* XXX CD Sector Size ??? */
	if (tracks > 0) {
		if (trackp->flags & TI_NOCD) {
	...
	for (i = 0; i <= tracks; i++) {
		...
		if (debug) {
			printf(_(
				...
				tp->filename, (int64_t)tp->itracksize,
				tp->isecsize,
	...
	track_t	*trackp;
{
	...
	if (trackp->itracksize < 0 &&
	    rb_regular_file_size_fd(f, &file_size)) {
		/* RB_RETROBURNER_TRACKSIZE_NARROW_GUARD_16 */
		trackp->itracksize = (tsize_t)file_size;
		if ((int64_t)trackp->itracksize != file_size)
			comerrno(EX_BAD,
			    "RetroBeam: input image size cannot be represented safely.\n");
	}
	if (trackp->itracksize >= 0) {
		/*
	...
	*flagsp |= F_WRITE;
	...
	for (; ; cac--, cav++) {
		...
		if (isize) {
			 ...
			 */
			flags |= TI_PAD;
			if (padsize == (int64_t)0L)
				padsize = (int64_t)PAD_SIZE;
		}
		...
		if (pad || ((flags & TI_AUDIO) == 0 && padsize > (int64_t)0L)) {
			flags |= TI_PAD;
			if ((flags & TI_AUDIO) == 0 && padsize == (int64_t)0L)
				padsize = (int64_t)PAD_SIZE;
		}
	...
	track_t	*trackp;
{
	...
	for (i = 0; i <= tracks+1; i++) {
		...
		/*
		 * RB_RETROBURNER_DVDR_32K_WRITES_10
		 *
		 * Keep the native Windows SPTI transport capable of 64 KiB
		 * transfers, but do not use 64 KiB WRITE(10) commands for the
		 * generic sequential DVD-R/DVD-RW writer.
		 *
		 * dvd+rw-tools/growisofs deliberately uses a 32 KiB DVD block
		 * strategy for DVD-R[W].  A real ASUS DVD-R test with 64 KiB
		 * WRITE(10) commands eventually ended in MEDIUM ERROR / WRITE
		 * ERROR despite a healthy FIFO and drive buffer, while growisofs
		 * successfully writes the same workload with 32 KiB commands.
		 *
		 * Scope:
		 *   - generic mmc_dvd driver only
		 *   - 2048-byte data sectors only
		 *   - exclude DVD-RAM
		 *
		 * CD, DVD+, BD and transport buffer capability are unchanged.
		 */
		if (dp != NULL &&
		    dp->cdr_drname != NULL &&
		    dp->cdr_dstat != NULL &&
		    strcmp(dp->cdr_drname, "mmc_dvd") == 0 &&
		    dp->cdr_dstat->ds_type != DST_DVD_RAM &&
		    secsize == 2048 &&
		    secspt > 16)
			secspt = 16;
	...
	track_t	*trackp;
{
	...
	error("xfp:		%p\n", trackp->xfp);
	error("filename:	%s\n", trackp->filename);
	error("itracksize:	%lld\n", (int64_t)trackp->itracksize);
	error("tracksize:	%lld\n", (int64_t)trackp->tracksize);
	error("trackstart:	%ld\n", trackp->trackstart);
	error("tracksecs:	%ld\n", trackp->tracksecs);
	error("padsecs:	%ld\n", trackp->padsecs);
	error("pregapsize:	%ld\n", trackp->pregapsize);
