...
EXPORT	BOOL	get_mode_params	__PR((SCSI *scgp, int page, char *pagename,
					...
					int *lenp));
EXPORT	BOOL	set_mode_params	__PR((SCSI *scgp, char *pagename, unsigned char *modep,
...
/* Maxoptix bringt Aborted cmd 0x0B mit code 0x4E (overlapping cmds)*/
	...
	if (scgp->verbose)
		scg_prbytes("Mode Page  Data", (unsigned char *)mp, mp->p_len+2);
...
get_mode_params(scgp, page, pagename, modep, cmodep, dmodep, smodep, lenp)
	...
	char	*pagename;
	unsigned char	*modep;
	unsigned char	*cmodep;
	unsigned char	*dmodep;
