...
/*
 * RB_RETROBURNER_NATIVE_WIN32_64K_04
 *
 * ImgBurn blackbox evidence on the ASUS SDRW-08U9M-U B201 / SONY16D1 path
 * uses 32-sector (65536-byte) WRITE(10) transfers.  Native Windows therefore
 * requests a full 64 KiB SPTI buffer.  Keep the upstream 63 KiB default for
 * Cygwin and POSIX targets.
 */
#if defined(_WIN32) && !defined(__CYGWIN__)
#define	CDR_BUF_SIZE		(64*1024)
#else
...
/* RB_RETROBURNER_64BIT_TRACKSIZE_15 */
...
struct cdr_cmd {
	...
	int	(*cdr_reserve_track)	__PR((SCSI *scgp, unsigned long len));			/* reserve track for future use */
	int	(*cdr_write_trackdata)	__PR((SCSI *scgp, char * buf, long daddr, long bytecnt, int seccnt, BOOL islast));
	...
	int	(*cdr_opc)		__PR((SCSI *scgp, char * bp, int cnt, int doopc));	/* Do OPC */
...
extern	BOOL	get_mode_params	__PR((SCSI *scgp, int page, char *pagename,
					unsigned char *modep, unsigned char *cmodep,
					unsigned char *dmodep, unsigned char *smodep,
					int *lenp));
extern	BOOL	set_mode_params	__PR((SCSI *scgp, char *pagename, unsigned char *modep,
 ...
 */
extern	int	getnum		__PR((char *arg, long *valp));
extern	int	getllnum	__PR((char *arg, int64_t *lvalp));
...
extern	int	request_sense_b	__PR((SCSI *scgp, char * bp, int cnt));
extern	int	inquiry		__PR((SCSI *scgp, char *, int));
...
extern	int	write_xscsi	__PR((SCSI *scgp, char *, long, long, int));
extern	int	write_xg0	__PR((SCSI *scgp, char *, long, long, int));
extern	int	write_xg1	__PR((SCSI *scgp, char *, long, long, int));
extern	int	write_xg5	__PR((SCSI *scgp, char *, long, long, int));
...
extern	int	read_buffer	__PR((SCSI *scgp, char * bp, int cnt, int mode));
extern	int	write_buffer	__PR((SCSI *scgp, char *buffer, long length, int mode, int bufferid, long offset));
extern	int	read_subchannel	__PR((SCSI *scgp, char * bp, int track,
					int cnt, int msf, int subq, int fmt));
extern	int	read_toc	__PR((SCSI *scgp, char *, int, int, int, int));
extern	int	read_toc_philips __PR((SCSI *scgp, char *, int, int, int, int));
extern	int	read_header	__PR((SCSI *scgp, char *, long, int, int));
extern	int	read_disk_info	__PR((SCSI *scgp, char *, int));
...
extern	int	read_track_info	__PR((SCSI *scgp, char *, int type, int addr, int cnt));
extern	int	get_trackinfo	__PR((SCSI *scgp, char *, int type, int addr, int cnt));
extern	int	read_rzone_info	__PR((SCSI *scgp, char * bp, int cnt));
extern	int	reserve_tr_rzone __PR((SCSI *scgp, long size));
extern	int	read_dvd_structure __PR((SCSI *scgp, char * bp, int cnt, int mt, int addr, int layer, int fmt));
extern	int	send_dvd_structure __PR((SCSI *scgp, char * bp, int cnt, int fmt));
extern	int	send_opc	__PR((SCSI *scgp, char *, int cnt, int doopc));
...
extern	int	read_master_cue	__PR((SCSI *scgp, char * bp, int sheet, int cnt));
extern	int	send_cue_sheet	__PR((SCSI *scgp, char * bp, long size));
...
extern	int	mode_select	__PR((SCSI *scgp, unsigned char *, int, int, int));
extern	int	mode_sense	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
extern	int	mode_select_sg0	__PR((SCSI *scgp, unsigned char *, int, int, int));
extern	int	mode_sense_sg0	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
extern	int	mode_select_g0	__PR((SCSI *scgp, unsigned char *, int, int, int));
extern	int	mode_select_g1	__PR((SCSI *scgp, unsigned char *, int, int, int));
extern	int	mode_sense_g0	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
extern	int	mode_sense_g1	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
...
extern	int	read_scsi	__PR((SCSI *scgp, char *, long, int));
extern	int	read_g0		__PR((SCSI *scgp, char *, long, int));
extern	int	read_g1		__PR((SCSI *scgp, char *, long, int));
...
extern	int	scsi_cdr_write	__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
extern	struct cd_mode_page_2A * mmc_cap __PR((SCSI *scgp, unsigned char *modep));
...
#ifdef	_SCG_SCSITRANSP_H
extern	int	get_configuration	__PR((SCSI *scgp, char * bp, int cnt, int st_feature, int rt));
...
extern	int	scsi_get_perf_maxspeed	__PR((SCSI *scgp, unsigned long *readp, unsigned long *writep, unsigned long *endp));
extern	int	scsi_get_perf_curspeed	__PR((SCSI *scgp, unsigned long *readp, unsigned long *writep, unsigned long *endp));
extern	int	retrobeam_media_intelligence	__PR((SCSI *scgp));
extern	int	retrobeam_vendor_fingerprint	__PR((SCSI *scgp));
extern	int	retrobeam_awsc_control		__PR((SCSI *scgp));
extern	int	retrobeam_awsc_get_state	__PR((SCSI *scgp, int *supportedp, int *awscdp));
extern	int	retrobeam_awsc_set_state	__PR((SCSI *scgp, int awscd));
...
extern	int	get_format_capacities	__PR((SCSI *scgp, char * bp, int cnt));
extern	int	read_format_capacities	__PR((SCSI *scgp, char * bp, int cnt));
#endif	/* _SCG_SCSITRANSP_H */
#ifdef _SCSIMMC_H
...
#ifdef	_SCG_SCSITRANSP_H
extern struct ricoh_mode_page_30 * get_justlink_ricoh	__PR((SCSI *scgp, unsigned char *mode));
#endif

/*
 * isosize.c
 */
extern	int64_t	isosize		__PR((int f));
extern	int64_t	bisosize	__PR((char *bp, int len));
...
#endif
extern	void	fillsubch	__PR((track_t *trackp, unsigned char *sp, int secno, int nsecs));
extern	void	filltpoint	__PR((unsigned char *sub, int ctrl_adr, int point, msf_t *mp));
extern	void	fillttime	__PR((unsigned char *sub, msf_t *mp));
extern	void	qpto96		__PR((unsigned char *sub, unsigned char *subq, int dop));
extern	void	addrw		__PR((unsigned char *sub, unsigned char	*subrwptr));
extern	void	qwto16		__PR((unsigned char *subq, unsigned char *subptr));
extern	void	subrecodesecs	__PR((track_t *trackp, unsigned char *bp, int address, int nsecs));

/*
 * sector.c
 */
extern	int	encspeed	__PR((BOOL be_verbose));
extern	void	encsectors	__PR((track_t *trackp, unsigned char *bp, int address, int nsecs));
extern	void	scrsectors	__PR((track_t *trackp, unsigned char *bp, int address, int nsecs));
extern	void	encodesector	__PR((unsigned char *sp, int sectype, int address));
extern	void	fillsector	__PR((unsigned char *sp, int sectype, int address));
