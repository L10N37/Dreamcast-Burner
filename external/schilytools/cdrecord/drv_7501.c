...
struct cw7501_mode_page_21 {	/* MCN */
		MP_P_CODE;		/* parsave & pagecode */
	unsigned char	p_len;			/* 0x12 = 20 Bytes */
...
struct cw7501_mode_page_23 {	/* Dummy / Write information */
	...
	unsigned char	p_len;			/* 0x02 = 2 Bytes */
	unsigned char	res;
	unsigned char	autopg		: 1;
	unsigned char	dummy		: 1;
	unsigned char	res3_72		: 6;
};
...
struct cw7501_mode_page_24 {	/* CD-R Disk information */
	...
	unsigned char	res;
	unsigned char	appl_code[4];
	unsigned char	disk_id[4];
};
...
struct cw7501_nwa {
	unsigned char	nwa_length[2];
	unsigned char	nwa_res;
	unsigned char	nwa_trackno;
	unsigned char	nwa_nwa[4];
	unsigned char	nwa_freeblocks[4];
};
...
struct cw7501_cue {
	...
	unsigned char	cs_extension;		/* Reserved or MCN/ISRC		*/
...
LOCAL	int	cw7501_write		__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
...
LOCAL	int	cw7501_read_trackinfo	__PR((SCSI *scgp, unsigned char *bp, int count, int track, int mode));
LOCAL	int	cw7501_write_dao	__PR((SCSI *scgp, unsigned char *bp, int len, int disktype));
	...
	int	*speedp;
{
	...
	if (mode_select(scgp, (unsigned char *)&md, count, 0, scgp->inq->data_format >= 2) < 0)
		return (-1);
	...
	long	*ap;
{
	...
	/*
	 * XXX F�r TAO definitiv notwendig.
	 * XXX ABh�ngig von Auto-Pregap?
	 */
	/* XXX */ next_addr += 150;
	if (ap)
		*ap = next_addr;
	...
	track_t	*trackp;
{
	...
	scgp->silent++;
	ret = cw7501_write_dao(scgp, (unsigned char *)cp, ncue*8, disktype);
	scgp->silent--;
	free(cp);
	if (ret < 0) {
