...
struct sony_924_mode_page_20 {	/* mastering information */
	...
	unsigned char	p_len;			/* 0x06 = 6 Bytes */
	unsigned char	subcode_header_off;
	unsigned char	res3_0		: 1;
	unsigned char	speudo		: 1;
	unsigned char	res3_2		: 1;
	unsigned char	c2po		: 1;
	unsigned char	subcode_ecc	: 1;
	unsigned char	res3_567	: 3;
	unsigned char	res_4;
	unsigned char	cue_sheet_opt;
	unsigned char	res[2];
};
...
struct sony_924_mode_page_22 {	/* disk information */
		MP_P_CODE;		/* parsave & pagecode */
	unsigned char	p_len;			/* 0x1E = 30 Bytes */
	unsigned char	disk_style;
	unsigned char	disk_type;
	unsigned char	first_track;
	unsigned char	last_track;
	unsigned char	numsess;
	unsigned char	res_7;
	unsigned char	disk_appl_code[4];
	unsigned char	last_start_time[4];
	unsigned char	disk_status;
	unsigned char	num_valid_nra;
	unsigned char	track_info_track;
	unsigned char	post_gap;
	unsigned char	disk_id_code[4];
	unsigned char	lead_in_start[4];
	unsigned char	res[4];
};

struct sony_924_mode_page_23 {	/* track information */
		MP_P_CODE;		/* parsave & pagecode */
	unsigned char	p_len;			/* 0x22 = 34 Bytes */
	unsigned char	res_2;
	unsigned char	track_num;
	unsigned char	data_form;
	unsigned char	write_method;
	unsigned char	session;
	unsigned char	track_status;
	unsigned char	start_lba[4];
	unsigned char	next_recordable_addr[4];
	unsigned char	blank_area_cap[4];
	unsigned char	fixed_packet_size[4];
	unsigned char	res_24;
	unsigned char	starting_msf[3];
	unsigned char	res_28;
	unsigned char	ending_msf[3];
	unsigned char	res_32;
	unsigned char	next_rec_time[3];
};

struct sony_924_mode_page_31 {	/* drive speed */
		MP_P_CODE;		/* parsave & pagecode */
	unsigned char	p_len;			/* 0x02 = 2 Bytes */
	unsigned char	speed;
	unsigned char	res;
};
...
struct sony_write_parameter {
	unsigned char	res0;			/* Reserved (must be zero)	*/
	unsigned char	len;			/* Parameter length 0x32 == 52	*/
	unsigned char	res2;			/* Reserved (must be zero)	*/
#if defined(_BIT_FIELDS_LTOH)	/* Intel byteorder */
	unsigned char	res3_05		: 6;	/* Reserved			*/
	unsigned char	ms		: 2;	/* Multi session mode		*/
#else				/* Motorola byteorder */
	unsigned char	ms		: 2;	/* Multi session mode		*/
	unsigned char	res3_05		: 6;	/* Reserved			*/
#endif
	unsigned char	resx[12];
#if defined(_BIT_FIELDS_LTOH)	/* Intel byteorder */
	unsigned char	res16_06	: 7;	/* Reserved			*/
	unsigned char	mcval		: 1;	/* MCN valid			*/
#else				/* Motorola byteorder */
	unsigned char	mcval		: 1;	/* MCN valid			*/
	unsigned char	res16_06	: 7;	/* Reserved			*/
#endif
...
#if defined(_BIT_FIELDS_LTOH)	/* Intel byteorder */
	unsigned char	res32_06	: 7;	/* Reserved			*/
	unsigned char	icval		: 1;	/* ISRC valid			*/
#else				/* Motorola byteorder */
	unsigned char	icval		: 1;	/* ISRC valid			*/
	unsigned char	res32_06	: 7;	/* Reserved			*/
#endif
	unsigned char	isrc[15];
	unsigned char	subheader[4];
};
...
struct sony_cue {
	...
	unsigned char	cs_zero;		/* Reserved or MCN/ISRC		*/
...
LOCAL	int	write_start_sony	__PR((SCSI *scgp, char * bp, int size));
LOCAL	int	write_continue_sony	__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
...
#ifdef	__needed__
LOCAL	int	set_wr_parameter_sony	__PR((SCSI *scgp, char * bp, int size));
#endif
...
#else
	if (dsp->ds_diskid != (unsigned long)0xFFFFFFFF)
#endif
	...
	int	*speedp;
{
	...
	err = mode_select(scgp, (unsigned char *)&md, count, 0, 1);
	if (err < 0)
		return (err);
	...
	return (mode_select(scgp, (unsigned char *)&md, count, 0, 1));
}
	...
	int	tracktype;
{
	...
	if (!get_mode_params(scgp, page, _("CD track information"),
			(unsigned char *)mode, (unsigned char *)0, (unsigned char *)0, (unsigned char *)0, &len)) {
		dec_verbose();
		return (-1);
	}
	dec_verbose();
	...
	int	tracktype;
{
	...
	if (mode_select(scgp, (unsigned char *)mode, len, 0, scgp->inq->data_format >= 2) < 0) {
		return (-1);
	}
...
/*
 * XXX set write parameter f�r SAO mit Multi Session (948 only?)
	...
	track_t	*trackp;
{
	...
	scgp->silent++;
	ret  = write_start_sony(scgp, (char *)cp, ncue*8);
	scgp->silent--;
	free(cp);
	if (ret < 0) {
