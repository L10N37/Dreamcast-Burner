...
error("NWAv %d Next rec addr %d\n", rp->nwa_v, (int)a_to_u_4_byte(rp->next_recordable_addr));
	...
	if (read_dvd_structure(scgp, (char *)sp, len, 0, 0, 0, 0) < 0)
		return (drive_getdisktype(scgp, dp));
...
error("MAXBLO %d from phys end - phys start\n", (int)(a_to_u_3_byte(sp->phys_end) - a_to_u_3_byte(sp->phys_start) + 1));
	...
	end_lba = 0L;
	scsi_get_perf_maxspeed(scgp, (unsigned long *)NULL, (unsigned long *)NULL, &end_lba);
#ifdef	DVDPLUS_DEBUG
error("end_lba: %lu\n", end_lba);
#endif
	/*
...
/*
 * XXX rzone_size(trackp) mu� aufgerufen werden, daher mu�
	...
	int	fmtflags;
{
...
#ifdef	DVDPLUS_DEBUG
	error("Cap len: %d\n", len);
	scg_prbytes("Format cap:", (unsigned char *)cap_buf, len);
#endif
	...
	fillbytes((char *)fmt_buf, sizeof (fmt_buf), '\0');
...
#ifdef	DVDPLUS_DEBUG
	scg_prbytes("Format desc:", (unsigned char *)fmt_buf, 12);
#endif
	...
	int32_t	lbreak;
{
	...
	ret = read_dvd_structure(scgp, (char *)&lb, sizeof (lb), 0, 0, 0, 0x20);
	if (ret < 0)
		return (ret);
	...
	ret = send_dvd_structure(scgp, (char *)&lb, sizeof (lb), 0x20);
	if (ret < 0)
		return (ret);

	/*
	 * RetroBurner XGD3 / ImgBurn parity:
	 *
	 * ImgBurn reads DVD Structure 20h twice after sending the layer
	 * boundary.  Do the same and fail closed if the drive does not echo
	 * the exact Layer 0 capacity that we requested.
	 */
	{
		int i;
		...
		for (i = 0; i < 2; i++) {
			...
			ret = read_dvd_structure(scgp, (char *)&lb,
					sizeof (lb), 0, 0, 0, 0x20);
			if (ret < 0) {
				errmsgno(EX_BAD,
					_("Cannot verify DVD+R DL layer break after SEND DVD STRUCTURE.\n"));
				return (ret);
			}

			if (a_to_u_4_byte(lb.l0_area_cap) != l0_cap) {
				errmsgno(EX_BAD,
					_("DVD+R DL layer-break readback mismatch: requested %u, drive returned %u.\n"),
					l0_cap, a_to_u_4_byte(lb.l0_area_cap));
				return (-1);
			}
		}
