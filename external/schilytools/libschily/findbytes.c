...
#endif
{
	register	unsigned char	uval = (unsigned char)val;
	register const	unsigned char	*cp  = (unsigned char *)vp;
	register const	unsigned long	*lp;
	register	unsigned long	lval;
	register	unsigned long	lmask;
	register	unsigned long	magic_mask;
	...
	error	SIZE_LONG has unknown value
#endif
	for (lp = (const unsigned long *)cp; cnt >= sizeof (long);
	    cnt -= sizeof (long)) {
		/*
	...
	for (cp = (const unsigned char *)lp; --cnt >= 0; ) {
		if (*cp++ == uval)
			return ((char *)--cp);
	}
	return ((char *)NULL);
