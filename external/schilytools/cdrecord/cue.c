...
typedef struct state {
	...
	void	*xfp;		/* Open file			*/
	int64_t	trackoff;	/* Current offset in open file	*/
	int64_t	filesize;	/* Size of current open file	*/
	...
	char	*delim;
{
	...
	for (s = (unsigned char *)linep; (c = *s) != '\0'; s++) {
		if (c == '"') {
			quoted = !quoted;
...
linelen--;
		...
		if (!quoted && isspace(c))
			break;
		if (!quoted && strchr(delim, c) && s > (unsigned char *)linep)
			break;
	}
	wordendp = (char *)s;
	wordendc = (char)*s;
