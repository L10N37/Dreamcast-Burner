...
#if defined(__MINGW32__) && !defined(HAVE_READLINK)
/*
 * RB_MINGW_READLINK_STUB
 * This MinGW configuration has neither lstat() nor readlink(). Since lstat
 * already falls back to stat() above, POSIX symlink discovery is unavailable.
 * Keep resolvepath() conservative and report readlink as unsupported if that
 * branch is ever reached.
 */
LOCAL ssize_t
rb_mingw_readlink(path, buf, bufsiz)
	const char	*path;
	char		*buf;
	size_t		bufsiz;
{
...
#define	readlink	rb_mingw_readlink
#endif
