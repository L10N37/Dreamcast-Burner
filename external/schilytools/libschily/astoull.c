...
EXPORT	char *astoull	__PR((const char *s, uint64_t *l));
EXPORT	char *astoullb	__PR((const char *s, uint64_t *l, int base));
	...
	register int base;
{
	...
	maxmult = TYPE_MAXVAL(uint64_t) / base;
	for (; (c = *s) != 0; s++) {
		...
		if (digit < base) {
			if (ret > maxmult)
				goto overflow;
			ret *= base;
			if (TYPE_MAXVAL(uint64_t) - ret < digit)
				goto overflow;
			ret += digit;
		} else {
			break;
