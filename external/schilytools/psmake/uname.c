	...
	struct utsname	*ubuf;
{
	...
	/* RB_BOOTSTRAP_STRLCAT_SHIM: generated bootstrap copy only */
	if (iswow64())
		strlcpy(ubuf->sysname, "Win32-WOW64", sizeof (ubuf->sysname));
	else
		strlcpy(ubuf->sysname, "Win32", sizeof (ubuf->sysname));

	/* RB_BOOTSTRAP_GETHOSTNAME_SHIM: generated bootstrap copy only */
	{
		DWORD	rb_namelen = sizeof (ubuf->nodename);
		if (!GetComputerNameA(ubuf->nodename, &rb_namelen))
			ubuf->nodename[0] = '\0';
	}
