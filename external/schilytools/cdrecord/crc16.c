...
LOCAL	uint16_t	updcrc		__PR((unsigned int p_crc, uint8_t *cp, unsigned int cnt));
/*LOCAL	uint16_t	calcCRC		__PR((unsigned char *buf, unsigned int bsize));*/
EXPORT	uint16_t	calcCRC		__PR((unsigned char *buf, unsigned int bsize));
EXPORT	uint16_t	fillcrc		__PR((unsigned char *buf, unsigned int bsize));
EXPORT	uint16_t	flip_crc_error_corr	__PR((unsigned char *b, unsigned int bsize, unsigned int p_crc));
...
{
	register uint16_t	crc = p_crc;
	register unsigned int		btsize = bsize * BPB;
