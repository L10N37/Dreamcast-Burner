...
bool BurnEngine::Start(BurnRequest request) {
    {
        ...
        state_.status = request.checkOnly
            ? "Extracting and checking CDI track layout..."
            : "Verifying burner and blank CD-R...";
    }
...
void BurnEngine::Run(BurnRequest request) {
    const fs::path appDirectory = ExecutableDirectory();
    const fs::path cdirip = appDirectory / L"tools" / L"cdirip.exe";
    const fs::path cdrecord = appDirectory / L"tools" / L"cdrecord.exe";
    if (appDirectory.empty() || !fs::is_regular_file(cdirip)) {
        SetFailure("Missing tools\\cdirip.exe beside DreamcastBurner.exe.");
        return;
    }
    if (!request.checkOnly && !fs::is_regular_file(cdrecord)) {
        SetFailure("Missing tools\\cdrecord.exe beside DreamcastBurner.exe.");
        return;
    }
