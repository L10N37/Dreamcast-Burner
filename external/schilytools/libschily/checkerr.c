...
typedef struct errconf {
	...
	const	unsigned char	*ec_pat;	/* File name pattern		    */
	...
	char	*line;
{
	...
	if (ep->ec_plen > maxplen)
		maxplen = ep->ec_plen;
	ep->ec_pat = (const unsigned char *)___savestr(pattern);
	ep->ec_aux = ___malloc(plen*sizeof (int), "compiled pattern");
	if ((ep->ec_alt = patcompile((const unsigned char *)pattern,
						plen, ep->ec_aux)) == 0)
	...
	const	char	*fname;
{
	ec_t		*ep = ec_root;
	char		*ret;
	const unsigned char	*name = (const unsigned char *)fname;
	int		nlen;
