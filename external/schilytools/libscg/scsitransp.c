	...
	int	maxcnt;
{
	...
	if (cp->scb.chk) {
		amt = scg_sprsense(p, maxcnt, (unsigned char *)&cp->sense, cp->sense_count);
		if (amt < 0)
			return (amt);
		p += amt;
		maxcnt -= amt;
	...
	int	maxcnt;
{
	...
	amt = js_snprintf(p, maxcnt, "status: 0x%x ", *(unsigned char *)&cp->scb);
	if (amt < 0)
		return (amt);
	p += amt;
	maxcnt -= amt;
	...
	if (cp->scb.ext_st1) {
		amt = js_snprintf(p, maxcnt, "0x%x ", ((unsigned char *)&cp->scb)[1]);
		if (amt < 0)
			return (amt);
		p += amt;
		maxcnt -= amt;
	...
	if (cp->scb.ext_st2) {
		amt = js_snprintf(p, maxcnt, "0x%x ", ((unsigned char *)&cp->scb)[2]);
		if (amt < 0)
			return (amt);
		p += amt;
		maxcnt -= amt;
...
#endif
	switch (*(unsigned char *)&cp->scb & 036) {
