...
EXPORT	int	log_sense		__PR((SCSI *scgp, char * bp, int cnt, int page, int pc, int pp));
EXPORT	BOOL	has_log_page		__PR((SCSI *scgp, int page, int pc));
EXPORT	int	get_log			__PR((SCSI *scgp, char * bp, int *lenp, int page, int pc, int pp));
	...
	int	pp;	/* Parameter Pointer */
{
	...
	if (scg_cmd(scgp) < 0)
		return (-1);
	if (scgp->verbose)
		scg_prbytes(_("Log sense Data   :"), (unsigned char *)bp, cnt - scg_getresid(scgp));
	return (0);
}
	...
	int	pc;
{
	...
	scgp->silent++;
	if (log_sense(scgp, (char *)log, sizeof (scsi_log_hdr), 0, pc, 0) < 0) {
		scgp->silent--;
		return (FALSE);
	}
	...
	if (log_sense(scgp, (char *)log, len + sizeof (scsi_log_hdr), 0, pc, 0) < 0) {
		scgp->silent--;
		return (FALSE);
	}
	scgp->silent--;
	...
	int	pp;
{
	...
	if (log_sense(scgp, (char *)log, sizeof (scsi_log_hdr), page, pc, pp) < 0)
		return (-1);
	...
	SCSI	*scgp;
{
	...
	scgp->silent++;
	if (log_sense(scgp, (char *)log, sizeof (scsi_log_hdr), 0, LOG_CUMUL, 0) < 0) {
		scgp->silent--;
		return;
	}
	...
	if (log_sense(scgp, (char *)log, len + sizeof (scsi_log_hdr), 0, LOG_CUMUL,  0) < 0) {
		scgp->silent--;
		return;
	}
	scgp->silent--;
