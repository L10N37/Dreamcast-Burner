...
LOCAL	void	print_bd00		__PR((unsigned char *dp));
LOCAL	void	print_bd09		__PR((unsigned char *dp));
LOCAL	void	print_bd0A		__PR((unsigned char *dp));
...
extern	char	*buf;
	...
	if ((dp->cdr_dstat->ds_cdrflags & RF_PRATIP) != 0) {
		if (((dsp->ds_cdrflags & (RF_WRITE|RF_BLANK)) == 0) ||
				lverbose > 1) {
			/*
			 * Das BD Medieninfo ist so lang, da� wir es
...
error("NWAv %d Next rec addr %d\n", rp->nwa_v, (int)a_to_u_4_byte(rp->next_recordable_addr));
	 ...
	 */
	fillbytes((char *)mode, sizeof (mode), '\0');
	if (read_dvd_structure(scgp, (char *)mode, 2, 1, 0, 0, 0) < 0) {
	...
	read_dvd_structure(scgp, (char *)sp, len, 1, 0, 0, 0);
/*	if (lverbose > 1)*/
/*		print_dvd00(sp);*/
	/*
...
#ifdef	BD_DEBUG
	error(_("Cap len: %d\n"), len);
	scg_prbytes(_("Format cap:"), (unsigned char *)cap_buf, len);
#endif
...
#ifdef	BD_DEBUG
	scg_prbytes(_("Format desc:"), (unsigned char *)fmt_buf, 12);
#endif
	...
	SCSI	*scgp;
{
	...
	if (lverbose > 1) {
		...
		for (i = 0; i <= 255; i++) {
			fillbytes((char *)mode, sizeof (mode), '\0');
			ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 1, 0, 0, i);
...
/*		printf("Page: %d ret: %d len: %d\n", i, ret, sizeof (mode) - scg_getresid(scgp));*/
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 1, 0, 0, 0);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
...
/*			scg_prascii("BD structure[0]: ", mode, sizeof (mode) - scg_getresid(scgp));*/
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 1, 0, 0, 0x09);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 1, 0, 0, 0x0A);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
	...
	fillbytes((char *)mode, sizeof (mode), '\0');
	scgp->silent++;
	ret = read_dvd_structure(scgp, (char *)mode, sizeof (mode), 1, 0, 0, 0xC0);
	scgp->silent--;
	if (ret >= 0) {
		if (lverbose > 2) {
...
print_bd00(dp)
	unsigned char	*dp;
{
	unsigned char	*bd = &((unsigned char *)dp)[4];
#ifdef	BD_DEBUG
