...
EXPORT	int	request_sense_b	__PR((SCSI *scgp, char * bp, int cnt));
EXPORT	int	inquiry		__PR((SCSI *scgp, char *, int));
...
EXPORT	int	write_xscsi	__PR((SCSI *scgp, char *, long, long, int));
EXPORT	int	write_xg0	__PR((SCSI *scgp, char *, long, long, int));
EXPORT	int	write_xg1	__PR((SCSI *scgp, char *, long, long, int));
EXPORT	int	write_xg5	__PR((SCSI *scgp, char *, long, long, int));
...
EXPORT	int	read_buffer	__PR((SCSI *scgp, char * bp, int cnt, int mode));
EXPORT	int	write_buffer	__PR((SCSI *scgp, char *buffer, long length, int mode, int bufferid, long offset));
EXPORT	int	read_subchannel	__PR((SCSI *scgp, char * bp, int track,
					int cnt, int msf, int subq, int fmt));
EXPORT	int	read_toc	__PR((SCSI *scgp, char *, int, int, int, int));
EXPORT	int	read_toc_philips __PR((SCSI *scgp, char *, int, int, int, int));
EXPORT	int	read_header	__PR((SCSI *scgp, char *, long, int, int));
EXPORT	int	read_disk_info	__PR((SCSI *scgp, char *, int));
EXPORT	int	read_track_info	__PR((SCSI *scgp, char *, int type, int addr, int cnt));
EXPORT	int	get_trackinfo	__PR((SCSI *scgp, char *, int type, int addr, int cnt));
EXPORT	int	read_rzone_info	__PR((SCSI *scgp, char * bp, int cnt));
EXPORT	int	reserve_tr_rzone __PR((SCSI *scgp, long size));
EXPORT	int	read_dvd_structure __PR((SCSI *scgp, char * bp, int cnt, int mt, int addr, int layer, int fmt));
EXPORT	int	send_dvd_structure __PR((SCSI *scgp, char * bp, int cnt, int fmt));
EXPORT	int	send_opc	__PR((SCSI *scgp, char *, int cnt, int doopc));
EXPORT	int	read_track_info_philips	__PR((SCSI *scgp, char *, int, int));
EXPORT	int	scsi_close_tr_session __PR((SCSI *scgp, int type, int track, BOOL immed));
EXPORT	int	read_master_cue	__PR((SCSI *scgp, char * bp, int sheet, int cnt));
EXPORT	int	send_cue_sheet	__PR((SCSI *scgp, char * bp, long size));
...
EXPORT	int	mode_select	__PR((SCSI *scgp, unsigned char *, int, int, int));
EXPORT	int	mode_sense	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
EXPORT	int	mode_select_sg0	__PR((SCSI *scgp, unsigned char *, int, int, int));
EXPORT	int	mode_sense_sg0	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
EXPORT	int	mode_select_g0	__PR((SCSI *scgp, unsigned char *, int, int, int));
EXPORT	int	mode_select_g1	__PR((SCSI *scgp, unsigned char *, int, int, int));
EXPORT	int	mode_sense_g0	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
EXPORT	int	mode_sense_g1	__PR((SCSI *scgp, unsigned char *dp, int cnt, int page, int pcf));
...
EXPORT	int	read_scsi	__PR((SCSI *scgp, char *, long, int));
EXPORT	int	read_g0		__PR((SCSI *scgp, char *, long, int));
EXPORT	int	read_g1		__PR((SCSI *scgp, char *, long, int));
...
EXPORT	int	scsi_cdr_write	__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
EXPORT	struct cd_mode_page_2A * mmc_cap __PR((SCSI *scgp, unsigned char *modep));
	...
	SCSI	*scgp;
{
	...
	scg_prsense((unsigned char *)sensebuf, CCS_SENSE_LEN - scg_getresid(scgp));
	return (0);
}
	...
	int	cnt;
{
	...
	if (scg_cmd(scgp) < 0)
		return (-1);
	if (scgp->verbose)
		scg_prbytes(_("Inquiry Data   :"), (unsigned char *)bp, cnt - scg_getresid(scgp));
	return (0);
}
...
#ifdef	DEBUG
	if (lverbose > 1)
		scg_prbytes(_("Track info:"), (unsigned char *)bp,
...
read_master_cue(scgp, bp, sheet, cnt)
	SCSI	*scgp;
	char *	bp;		/* address of master cue sheet	*/
...
send_cue_sheet(scgp, bp, size)
	SCSI	*scgp;
	char *	bp;		/* address of cue sheet buffer */
	long	size;		/* number of bytes to transfer */
{
	register struct	scg_cmd	*scmd = scgp->scmd;
	...
	int	pcf;
{
	...
	fillbytes((char *)xmode, sizeof (xmode), '\0');
	if (amt < 4) {		/* Data length. medium type & VU */
		amt += 1;
	} else {
		amt += 4;
	...
	long	*lop;
{
	...
	if (scgp->verbose) {
		scg_prbytes(_("TOC data: "), (unsigned char *)xb,
		...
		while ((char *)tp < pe) {
			scg_prbytes(_("ENT: "), (unsigned char *)tp, 11);
			tp++;
		}
	}
	tp = &dp->desc[0];
	...
	for (; (char *)tp < pe; tp++) {
		...
		if (tp->point != 0xB0)
			continue;
		if (scgp->verbose)
			scg_prbytes("B0: ", (unsigned char *)tp, 11);
		if (isbcd) {
	...
	long	*offp;
{
	...
	fillbytes((char *)xb, sizeof (xb), '\0');
	if (read_toc(scgp, (char *)xb, 0, sizeof (struct tocheader), 0, FMT_SINFO) < 0)
		return (-1);

	if (scgp->verbose)
		scg_prbytes(_("tocheader: "),
		(unsigned char *)xb, sizeof (struct tocheader) - scg_getresid(scgp));
	...
	if (read_toc(scgp, (char *)xb, 0, len, 0, FMT_SINFO) < 0)
		return (-1);

	if (scgp->verbose)
		scg_prbytes(_("tocheader: "),
			(unsigned char *)xb, len - scg_getresid(scgp));
	...
	long	*offp;
{
	...
	fillbytes((char *)xb, sizeof (xb), '\0');
	if (read_toc_philips(scgp, (char *)xb, 0, sizeof (struct siheader), 0, FMT_SINFO) < 0)
		return (-1);
	...
	if (read_toc_philips(scgp, (char *)xb, 0, len, 0, FMT_SINFO) < 0)
		return (-1);
	/*
	...
	BOOL	print;
{
	...
	fillbytes((char *)inq, sizeof (*inq), '\0');
...
/*	if (scmd->error < SCG_FATAL || scmd->scb.chk && scmd->sense_count > 0){*/
	...
	if (inquiry(scgp, (char *)inq, sizeof (*inq)) < 0) {
		got_inquiry = FALSE;
	} else {
...
/*				scgp->dev = acbdev();*/
		...
		unsigned char	ibuf[256+5];
		unsigned char	*ip = (unsigned char *)inq;
		unsigned char	c;

		if (len > (int)sizeof (*inq) &&
				inquiry(scgp, (char *)ibuf, inq->add_len+5) >= 0) {
	...
	case INQ_DASD:
		if (inq->add_len == 0 && inq->inq_vendor_info[0] != '\0') {
			...
			for (p = (unsigned char *)&inq->inq_vendor_info[0];
					p < (unsigned char *)&inq->inq_prod_revision[4];
									p++) {
				if (*p < 0x20 || *p > 0x7E) {
					inq->add_len = 0;
					break;
	...
	FILE	*f;
{
	...
	if (inq->add_len >= 31 ||
		...
		fprintf(f, _("Identification : '%.16s'\n"), inq->inq_prod_ident);
