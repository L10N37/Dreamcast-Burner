...
LOCAL	int	scsi_cdr_write_philips		__PR((SCSI *scgp, char * bp, long sectaddr, long size, int blocks, BOOL islast));
...
/*			printf("lead-in len: %d lead-out len: %d\n", lilen, lolen);*/
	...
	if (read_subchannel(scgp, sbuf, 0, 14, 0, 0, 0xf1) >= 0)
		scg_prbytes(_("Disk bar code:"), (unsigned char *)sbuf, 14 - scg_getresid(scgp));
	scgp->silent--;
	...
	int	npa;
{
	...
	fillbytes((char *)&fwa, sizeof (fwa), '\0');
	fillbytes((char *)scmd, sizeof (*scmd), '\0');
	scmd->addr = (char *)&fwa;
	...
	long	*lolenp;
{
	...
	if (scgp->verbose)
		scg_prbytes(_("Session info data: "), (unsigned char *)buf, sizeof (buf) - scg_getresid(scgp));
