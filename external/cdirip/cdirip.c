...
if (flags.ask_for_image)
   ...
   if (strlen(argv[1]) >= sizeof(filename))
      error_exit(ERR_GENERIC, "Source CDI path is too long");
...
if (fsource == NULL)
   ...
   if (errno == ENOENT)  // "No such file or directory"
      {
      if (strlen(filename) + strlen(".cdi") >= sizeof(filename))
         error_exit(ERR_GENERIC, "Source CDI path is too long");
      strcat(filename, ".cdi");
...
if (!opts.showinfo)
   ...
   if (!(!flags.ask_for_image && flags.ask_for_dest_path))
      ...
      if (flags.ask_for_dest_path)
         ...
         {
         if (strlen(argv[2]) >= sizeof(destpath))
            error_exit(ERR_GENERIC, "Destination path is too long");
