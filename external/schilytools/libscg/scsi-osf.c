	...
	SCSI		*scgp;
{
	...
	if (ioctl(scgp->fd, UAGT_CAM_IO, (char *) &ua) < 0) {
	...
	if (ccb.cam_ch.cam_status & CAM_SIM_QFRZN) {
		...
		if (ioctl(scgp->fd, UAGT_CAM_IO, (char *) & relua) < 0)
			errmsg("DEC CAM -> LMA\n");
	}
	return (0);
}
