...
/*
 * RB_MINGW_CREATEHARDLINK_FALLBACK
 * Modern MinGW does not expose POSIX link(). Use the equivalent Win32
 * CreateHardLinkA() operation for Schily's linkat() compatibility module.
 */
#include <schily/windows.h>

LOCAL int
rb_mingw_link(oldpath, newpath)
	const char	*oldpath;
	const char	*newpath;
{
