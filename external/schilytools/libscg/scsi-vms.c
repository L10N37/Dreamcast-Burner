...
struct SCSI$IOSB {
	...
	unsigned long	SCSI$L_IOSB_TFR_CNT;	/* Actual #bytes transferred */
...
static unsigned short	gk_chan;
static unsigned short	transfer_length;
static int	i;
static int	status;
	...
	struct scg_cmd	*sp;
{
	...
	fillbytes((char *)&s_cmd, sizeof (s_cmd), '\0');
	...
	SCSI		*scgp;
{
	...
	int	error = sp->error;
	unsigned char	status = sp->u_scb.cmd_scb[0];
	int	ret;
