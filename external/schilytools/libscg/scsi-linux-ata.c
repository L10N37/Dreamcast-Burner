...
static	uid_t		cureuid = 0;	/* XXX Hack until we have uid management */
	...
	/* initialize */
	fillbytes((char *) & sg_cgc, sizeof (sg_cgc), '\0');
	fillbytes((char *) & sense_cgc, sizeof (sense_cgc), '\0');
...
again:
	...
	if (ret < 0) {
		...
		if (sp->ux_errno == EINVAL) {
			 ...
			 * "Invalid parameter".
			 */
			if ((((unsigned char *)sg_cgc.sense)[0] != 0) ||
			    (((unsigned char *)sg_cgc.sense)[2] != 0))
				sp->ux_errno = EIO;
	...
	if (ret < 0 && sg_cgc.sense->error_code) {
		...
		log(("%s\n", tmp_sense));
#endif	/* DEBUG */
		memcpy(sp->u_sense.cmd_sense, /* (char *) */ &sense_cgc, SCG_MAX_SENSE);
