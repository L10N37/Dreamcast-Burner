	...
	int	f;
{
	...
	fillbytes((char *)&sg_rep, sizeof (struct sg_header), '\0');
...
static	uid_t		cureuid = 0;	/* XXX Hack until we have uid management */
	...
	fillbytes((char *)&sg_io, sizeof (sg_io), '\0');
	...
	SCSI		*scgp;
{
	 ...
	 */
	fillbytes((char *)&sg_rq, sizeof (struct sg_header), '\0');
...
/*	sg_rq.hd.result = 0;	not needed because of fillbytes() */
	...
	if ((char *)&sg_rq != (char *)sgp)
		movebytes((char *)&sg_rq, (char *)sgp, sizeof (struct sg_header));
#else
	fillbytes((char *)sgp, sizeof (struct sg_header), '\0');
