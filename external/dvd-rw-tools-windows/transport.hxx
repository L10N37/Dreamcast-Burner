...
// settled with Inserve Technology, �v�gen 6, 412 50 G�TEBORG, Sweden,
...
public:
	...
	const int rb_trace_write10 =
	    (p.spt.Cdb[0] == 0x2A || p.spt.Cdb[0] == 0x2E);
	DWORD rb_trace_started = 0;
	if (rb_trace_write10)
	{
	    unsigned long rb_lba =
		((unsigned long)p.spt.Cdb[2] << 24) |
		((unsigned long)p.spt.Cdb[3] << 16) |
		((unsigned long)p.spt.Cdb[4] << 8) |
		(unsigned long)p.spt.Cdb[5];
	    unsigned long rb_blocks =
		((unsigned long)p.spt.Cdb[7] << 8) |
		(unsigned long)p.spt.Cdb[8];
	    unsigned long long rb_ptr =
		(unsigned long long)(ULONG_PTR)buf;

	    fprintf(stderr,
		"RBTRACE WRITE10 begin: LBA=%lu blocks=%lu bytes=%lu timeout=%lu "
		"buffer=0x%llX mod512=%llu mod4096=%llu\n",
		rb_lba,
		rb_blocks,
		(unsigned long)sz,
		(unsigned long)p.spt.TimeOutValue,
		rb_ptr,
		rb_ptr & 511ULL,
		rb_ptr & 4095ULL);
	    fflush(stderr);
	    rb_trace_started = GetTickCount();
	}
	...
	if (DeviceIoControl (fd,IOCTL_SCSI_PASS_THROUGH_DIRECT,
				...
				&bytes,FALSE) == 0)
	{
	    DWORD rb_error = GetLastError();
	    if (rb_trace_write10)
	    {
		DWORD rb_elapsed = GetTickCount() - rb_trace_started;
		fprintf(stderr,
		    "RBTRACE WRITE10 DeviceIoControl FAILED: elapsed_ms=%lu "
		    "winerr=%lu scsi_status=0x%02X "
		    "sense=%02X/%02X/%02X/%02X\n",
		    (unsigned long)rb_elapsed,
		    (unsigned long)rb_error,
		    (unsigned int)p.spt.ScsiStatus,
		    (unsigned int)p.sense[0],
		    (unsigned int)(p.sense[2] & 0x0F),
		    (unsigned int)p.sense[12],
		    (unsigned int)p.sense[13]);
		fflush(stderr);
	    }
	    SetLastError(rb_error);
	    return -1;
	}
	...
	if (rb_trace_write10)
	{
	    DWORD rb_elapsed = GetTickCount() - rb_trace_started;
	    fprintf(stderr,
		"RBTRACE WRITE10 completed: elapsed_ms=%lu scsi_status=0x%02X "
		"sense=%02X/%02X/%02X/%02X\n",
		(unsigned long)rb_elapsed,
		(unsigned int)p.spt.ScsiStatus,
		(unsigned int)p.sense[0],
