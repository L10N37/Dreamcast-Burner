...
EXPORT	void	fillsubch	__PR((track_t *trackp, unsigned char *sp, int secno, int nsecs));
EXPORT	void	filltpoint	__PR((unsigned char *sub, int ctrl_adr, int point, msf_t *mp));
EXPORT	void	fillttime	__PR((unsigned char *sub, msf_t *mp));
LOCAL	void	filldsubq	__PR((unsigned char *sub, int ca, int t, int i, msf_t *mrp, msf_t *mp));
LOCAL	void	fillmcn		__PR((unsigned char *sub, unsigned char *mcn));
LOCAL	void	fillisrc	__PR((unsigned char *sub, unsigned char *isrc));
LOCAL	int	ascii2q		__PR((int c));
LOCAL	void	qpto16		__PR((unsigned char *sub, unsigned char *subq, int dop));
EXPORT	void	qpto96		__PR((unsigned char *sub, unsigned char *subq, int dop));
EXPORT	void	addrw		__PR((unsigned char *sub, unsigned char	*subrwptr));
EXPORT	void	qwto16		__PR((unsigned char *subq, unsigned char *subptr));
EXPORT	void	subrecodesecs	__PR((track_t *trackp, unsigned char *bp, int address, int nsecs));
#ifdef	DO_SUBINTERLEAVE
LOCAL	void	subinterleave	__PR((unsigned char *sub));
#endif
...
struct q {
	unsigned char ctrl_adr;	/*  0 (ctrl << 4) | adr		 */
	unsigned char track;	/*  1 current track #		 */
	unsigned char index;	/*  2 current index #		 */
	unsigned char pmin;	/*  3 Relative time minutes part */
	unsigned char psec;	/*  4 Relative time seconds part */
	unsigned char pframe;	/*  5 Relative time frames part  */
	unsigned char zero;	/*  6 */
	unsigned char amin;	/*  7 Absolute time minutes part */
	unsigned char asec;	/*  8 Absolute time seconds part */
	unsigned char aframe;	/*  9 Absolute time frames part  */
	unsigned char crc1;	/* 10	all bits inverted. Polynom is	*/
	unsigned char crc2;	/* 11	X^16 + X^12 + X^5 + 1		*/
};

EXPORT	unsigned char	_subq[110][12];
EXPORT	int	_nsubh;
...
fillsubch(trackp, sp, secno, nsecs)
	track_t	*trackp;
	unsigned char	*sp;	/* Sector data pointer	*/
...
/*
 * Fill MCN
 * MCN konvertieren und in Sub-Q einf�llen.
 */
LOCAL void
fillmcn(sub, mcn)
	unsigned char	*sub;
...
/*
 * Fill ISRC
 * ISRC konvertieren und in Sub-Q einf�llen.
 */
LOCAL void
fillisrc(sub, isrc)
	unsigned char	*sub;
...
/*
 * Q-Sub auf 16 Bytes bl�hen und P-Sub addieren
 *
 * OUT: sub, IN: subqptr
 */
LOCAL void
...
EXPORT void
addrw(sub, subrwptr)
	register unsigned char	*sub;
	register unsigned char	*subrwptr;
{
	register int	i;
...
unsigned char	tq[12] = { 0x01, 0x00, 0xA0, 0x98, 0x06, 0x12, 0x00, 0x01, 0x00, 0x00, 0xE3, 0x74 };
...
testcrc()
{
	...
	ocrc = calcCRC((unsigned char *)&q, 12);
	printf("AC: %02X t: %3d (%02X) i: %3d (%02X) %d:%d:%d %d:%d:%d %02X%02X %X (%X)\n",
		...
		crc1, crc2,
		ocrc,
		fillcrc((unsigned char *)&q, 12) & 0xFFFF);
}
#endif	/* TEST_CRC */
